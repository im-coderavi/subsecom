    ...
    <div className="w-full min-h-screen flex items-center justify-center pt-12 pb-16 px-4 relative">
      ...
      <div className="w-full max-w-md p-6 sm:p-10 rounded-3xl bg-surface border border-line shadow-2xl shadow-black/40 relative z-10">
        <div className="text-center mb-8">
          ...
          <h2 className="text-lg sm:text-xl font-extrabold text-white tracking-tight">Create Your Account</h2>
          <p className="text-xs text-slate-400 mt-1 font-semibold">Join 15,000+ creators and unlock premium AI tools.</p>
        </div>
        ...
        <form onSubmit={handleRegisterSubmit} className="space-y-4">
          <div className="flex flex-col gap-1.5">
            ...
            <div className="relative">
              <input type="text" required value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. Sarah Jenkins"
              ...
              <LucideIcon name="User" size={13} className="text-slate-500 absolute left-3.5 top-3.5" />
            </div>
          </div>
          ...
          <button type="submit" disabled={isLoading}
            ...
            {isLoading
              ? <><LucideIcon name="RefreshCcw" size={13} className="animate-spin" /> Creating account...</>
              : <><LucideIcon name="UserPlus" size={14} /> Create Account</>}
          </button>
        </form>

        <p className="text-center mt-6 text-xs text-slate-400 font-bold">
          Already have an account?{' '}
          <Link to="/login" className="text-brand-400 hover:underline font-black">Sign in</Link>
        </p>
      </div>
    </div>
  );
